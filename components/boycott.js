...
function BoycottSeeAll({ route }) {
  ...
  const companies = useStore((state) => state.boycottedCompanies);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (querySubmitted) {
          setShowText(filteredBoycottedCompanies.length === 0 && searchQuery.length > 0);
